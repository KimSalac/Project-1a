    ...
    while (reg_file.pc != end_pc) {
        ...
        { //if I type
          ...
          else if(op == 0b101000 || op == 0b101001 || op == 0b101011) // sb, sh, sw
          {
            ...
            //cout << "rs data for stores: " << data_rs << endl;
          }
    ...
    while (current_state.memwb.complete == 0 ) {
      ...
      if (current_state.pc_write == 1){//(current_state.ifid.instruction == 0){ //check for fetch stage? (or use pc write)
       ...
       //decode(in);
        ...
        if (next_state.pc  == end_pc) //if the last instruction has been fetched...
        {  
      ...
      {
        next_state.ifid.ifid_write = 0; //if no instruction is fetched, then dont decode anything also for the next stage
      }
      /*cout << "\n" << "STATE OF REGISTER FILE AFTER FETCH STAGE: " << endl;
      reg_file.print(); */
      ...
      cout<<"------------Decode: ---------------"<<endl;
       ...
       // cout << "Instruction in decode: "<< instruction << endl;
        ...
        if(next_state.idex.control.branch == 1) // detect if its a branch, if it is, save the pc of its instruction
        {
          next_state.idex.branch_pc = decoded_pc; //if instruction that was just decoded is a branch, save its pc and move it into execution
          //cout << "Branch pc is: " << next_state.idex.branch_pc << endl;
        }
        else
        {
          next_state.idex.branch_pc = 0;
        }
        ...
        /****get opcode & funct: good ****/
        uint32_t op = instruction >> 26; // gets op       
        next_state.idex.op = op; // set the opcode to be used for execution in the next state
        
        uint32_t funct = instruction << 26; // gets funct (see next too)
        funct = funct >>26;
        next_state.idex.funct = funct; //set the funct to be used for execution in the next state
          ...
          { //if r-type
            ...
            rd_b = rd_b >> 27; // isolate rd
            rd_num = (int32_t) rd_b; //convert rd to int
            next_state.idex.rd = rd_num; //set rd to be used for execution in the next state
            ...
            if (instruction != 0) //check for branch delay slot; if this decoded instruction here needs CURRENT STAGE'S ALU
            { 
                if(next_state.idex.rs == current_state.idex.rd && current_state.idex.op==0) // forwarding rs from an R-type ALU
                {
                  current_state.forwardrs = 1;
                }
                if(next_state.idex.rt == current_state.idex.rd && current_state.idex.op==0) // forwarding rs from an R-type ALU
                {
                  current_state.forwardrt = 1;
                }
                if(next_state.idex.rs == current_state.idex.rt && current_state.idex.control.mem_write == 0 && current_state.idex.control.branch == 0 && current_state.idex.op != 0) // forwarding from an I-type ALU
                {  
                  //cout << "rs = rt: " << (next_state.idex.rs == current_state.idex.rt) << endl;
                  //cout << "rt is not from a branch instruction: " << (current_state.idex.control.branch == 0) << endl;
                  //cout << "rt is an I-type instruction: " << (current_state.idex.op != 0) << endl;

                  //cout << "GOT HERE 1" << endl;
                  current_state.forwardrs = 1; //forward the result of alu to rs
                }
                if(next_state.idex.rt == current_state.idex.rt && current_state.idex.control.mem_write == 0 && current_state.idex.control.branch == 0 && current_state.idex.op != 0) // forwarding from an I-type ALU
                {  
          ...
          { //if I type 
            ...
            //hazard logic for Itype; if this decoded instruction here needs CURRENT STAGE'S ALU
            if(instruction != 0) //check for branch delay slot
            {

              if(next_state.idex.rs == current_state.idex.rt && current_state.idex.control.branch == 0 && current_state.idex.control.mem_write == 0 && current_state.idex.op != 0) // forwarding from an I-type ALU
              {  
                //cout << "GOT HERE" << endl;
                cout<<"forward rs"<<endl;
                  current_state.forwardrs = 1; //forward the result of alu to rs
              }
              if(next_state.idex.rs == current_state.idex.rd && current_state.idex.op==0) // forwarding from an R-type ALU
              {
                cout<<"forward rs"<<endl;
                  current_state.forwardrs = 1;
              }
            ...
            { //change back b/c of branch delay slot
                current_state.forwardrs = 0;
                current_state.forwardrt = 0;
            }
            ...
            else if(op == 0b101000 || op == 0b101001 || op == 0b101011) // sb, sh, sw
            {
              ...
              //sw forward
              /*if(instruction != 0){ //check for branch delay slot
                if((next_state.idex.rs == current_state.idex.rt && current_state.idex.op != 0)
                || (current_state.idex.rs == next_state.idex.rd && next_state.idex.op == 0)){
            ...
            else if(control.branch)
            {
              ...
              next_state.idex.data_rt = data_rt;
              //cout << "\nvalue in R[rs_num]: ";
              //reg_file.print(rs_num);
              //cout << "\nvalue in R[rt_num]: ";
              //reg_file.print(rt_num);
              //cout<<"data_rs to be moved into execution from decode: "<< data_rs <<endl;
              //cout<<"data_rt to be moved into execution from decode: "<< data_rt << endl;
            }
            else // rest of regular arithmetic and loads
        ...
        //cout << "data_rs in decode: " << data_rs;

        /******I THINK THIS IS GOOD*******/
        //HANDLES IMPLICIT REGISTER FOREWARDING (if this decode requires memory that is being written back right now)
        //make sure data hazards in mem are not branches b/c we dont want to set values based on branches
        if(instruction != 0){ //check for branch delay slot
           //if source rt (when it is an R-type or a branch) equals destination rd (when it is an R-type)
            if(next_state.idex.rs == current_state.memwb.rt && (current_state.memwb.control.mem_write == 0 && current_state.memwb.control.branch == 0 && current_state.memwb.op != 0))
            {
                next_state.idex.data_rs = current_state.memwb.write_data;
            }
            if(next_state.idex.rt == current_state.memwb.rt && (next_state.idex.control.mem_write==1 || next_state.idex.control.branch == 1 || next_state.idex.op == 0) && (current_state.memwb.control.mem_write == 0 && current_state.memwb.control.branch == 0 && current_state.memwb.op != 0))
            { 
               // cout<<"current state memwb write data: "<<current_state.memwb.write_data<<endl;
                next_state.idex.data_rt = current_state.memwb.write_data;
            }
            if(next_state.idex.rs == current_state.memwb.rd && current_state.memwb.op == 0)
            { 
               //cout<<"got here 3"<<endl;
                next_state.idex.data_rs = current_state.memwb.write_data; 
            }
            if(next_state.idex.rt == current_state.memwb.rd && (current_state.memwb.op == 0) && (next_state.idex.control.mem_write==1 || next_state.idex.op == 0 || next_state.idex.control.branch == 1))
            {
        ...
        //HANDLES DATA HAZARDS WHEN GOING FROM MEMORY TO EXECUTION (more implicit forwarding)
        if(instruction != 0){ //check for branch delay slot
           //if source rt (when it is an R-type or a branch) equals destination rd (when it is an R-type)
            ...
            if(next_state.idex.rt == current_state.exmem.rt && (next_state.idex.control.mem_write==1 || next_state.idex.control.branch == 1 || next_state.idex.op == 0) && (current_state.exmem.control.mem_write==0 && current_state.exmem.control.branch == 0 && current_state.exmem.op != 0))
            { 
               // cout<<"current state memwb write data: "<<current_state.memwb.write_data<<endl;
                next_state.idex.data_rt = current_state.exmem.write_data;
            }
        ...
        current_state.idex.print();

      }
      else{
        next_state.idex.idex_write = 0; //else dont do this stage again
      }

      //cout << "\n" << "STATE OF REGISTER FILE AFTER DECODE STAGE: " << endl;
      //reg_file.print();
      ...
      if(current_state.idex.idex_write == 1){
        ...
        //cout << "DATARS RIGHT BEFORE EXECUTION: " << current_state.idex.data_rs << endl;
        ...
        uint32_t branch_pc = current_state.idex.branch_pc;
        next_state.exmem.instruction = current_state.idex.instruction;
        ...
        if (op == 0 && !(funct == 8)) //if rtype except jumpreg
        { 
          //cout << "GOT HERE TO R-TYPE" << endl;
          if(funct == 2 || funct == 0) // if shift
          {
            alu_result = alu.execute(data_rt, shamt, alu_zero);
          }
        ...
        {
          //cout << "GOT HERE TO I-TYPE" << endl;
         if(!current_state.idex.control.branch)
          {
            alu_result = alu.execute(data_rs, data_i, alu_zero); // this calculates address for loads, and values for registers for all other instructions
            //cout << "DATA_RS value from execution" << data_rs << endl;
          }
          else
          {
            //cout << "data_rs: " << data_rs << endl;
        ...
        //cout<<"LW STALL"<<endl; 
          ...
          {
            //cout<<"STALL"<<endl;
            next_state.pc = current_state.pc - 4; //makes instruction at pc redo that instruction
        ...
        if(current_state.forwardrs == 1)
        {
          //cout<<"Got here and actually did the forward" <<endl;
          next_state.idex.data_rs = alu_result;
          cout << "FORWARDED THIS RS VALUE FROM EXECUTION TO NEXT INSTRUCTION DECODE: " << alu_result;
        }
        if (current_state.forwardrt == 1)
        {
          //cout<<"Got here 2" <<endl;
          next_state.idex.data_rt = alu_result;
         cout << "FORWARDED RT VALUE FROM EXECUTION TO NEXT INSTRUCTION DECODE: " << alu_result;
        }

        //branch pc update
        //cout << "IS THIS A BRANCH: " << current_state.idex.control.branch << endl;
         if(current_state.idex.control.branch) // update proper branch address
          {
              //cout << "IS THIS A BRANCH EQUALS: " << current_state.idex.control.beq << endl;
              //cout << "DID IT RETURN TRUE: " << alu_zero << endl;
              if((current_state.idex.control.beq==1 && alu_zero==1) || (current_state.idex.control.beq==0 && alu_zero==0)) // true if beq = 0 & alu_zero = 0 OR beq = 1 & alu_zero = 1
              {
                //cout<<"Current state pc: "<<current_state.pc<<endl;
                next_state.pc = current_state.idex.branch_pc + 4 + (data_i << 2); //is it current state or the pc of the branch instruction?
                //cout << "Amount of instructions being jumped: " << data_i << endl;
        ...
        //next_state.exmem.control.print();
        next_state.exmem.alu_result = alu_result; //put the data that needs to be written in alu_result
        //cout<<" alu_result (memory address) written to memory: " << alu_result << endl;
      ...
      /*cout << "\n" << "STATE OF REGISTER FILE AFTER EXECUTE STAGE: " << endl;
      reg_file.print(); */
      ...
      if(current_state.exmem.exmem_write == 1){
        //cout<<"------------Memory: ---------------"<<endl;
        uint32_t instruction = current_state.exmem.instruction;
        ...
        if(current_state.exmem.control.mem_read == 1 || current_state.exmem.control.mem_write == 1)
        {
          ...
          if(current_state.exmem.control.mem_write == 1) //stores
          {
            ...
            //actual handling of storing value in register into memory
          ...
          else // if(current_state.exmem.control.mem_to_reg > 0) //loads
          {
            ...
            //memory.print(alu_result/4, 1);
              ...
              if(next_state.idex.rs == current_state.exmem.rt && (current_state.exmem.control.mem_write==0 && current_state.exmem.control.branch == 0 && current_state.exmem.op != 0))
              {
                next_state.idex.data_rs = data_write;
              }
              if(next_state.idex.rt == current_state.exmem.rt && (next_state.idex.control.mem_write==1 || next_state.idex.control.branch == 1 || next_state.idex.op == 0) && (current_state.exmem.control.mem_write==0 && current_state.exmem.control.branch == 0 && current_state.exmem.op != 0))
              { 
                  // cout<<"current state memwb write data: "<<current_state.memwb.write_data<<endl;
                next_state.idex.data_rt = data_write;
              }
          }
      ...
      /*------------------------------WRITE BACK--------------------------------*/
       if(current_state.memwb.memwb_write == 1){ //if we should write to this stage
        ...
        //cout << "Instruction from writeback: " << current_state.memwb.instruction << endl;
        //cout<< "data being written: " << current_state.memwb.write_data << endl;

        if (current_state.memwb.control.reg_write == 1) // might need to fix
        {
          if(current_state.memwb.control.reg_dest == 0)//write to rt
          { 
           if(current_state.memwb.op  == 0b001111) 
           { //lui
              reg_file.access(0, 0, data_rs, data_rs, rt_num, 1, data_i);
            }
            else // all other operations
        ...
        if (next_state.pc_write == 0 
        && next_state.ifid.ifid_write == 0 
        && next_state.idex.idex_write == 0 
        && next_state.exmem.exmem_write == 0 
        && next_state.memwb.memwb_write == 0)
        { //if there is literally no other instructions in every pipeline register after this point, then we know this is the last instruction
